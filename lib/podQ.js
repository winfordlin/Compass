...
exports.isInspectAndFixApp = async () => {
  let answer = null;
  await inquirer
    .prompt({
      type: 'confirm',
      name: 'isInspectAndFixApp',
      message: `Did you inspect the logs and ${colors.red(
        'FIX'
      )} the crashing app?`,
      default: 'Y',
    })
    .then((ans) => (answer = ans.isInspectAndFixApp));
  return answer;
};

exports.isRunContainerError = async () => {
  let answer = null;
  await inquirer
    .prompt({
      type: 'confirm',
      name: 'isRunContainerError',
      message: `Is the Pod status ${colors.red('RunContainerError')}?`,
      default: 'Y',
    })
    .then((ans) => (answer = ans.isRunContainerError));
  return answer;
};

exports.isCMDDockerfile = async () => {
  let answer = null;
  await inquirer
    .prompt({
      type: 'confirm',
      name: 'isCMDDockerfile',
      message: `Did you forget the ${colors.red(
        'CMD'
      )} instructions in the Dockerfile?`,
      default: 'Y',
    })
    .then((ans) => (answer = ans.isCMDDockerfile));
  return answer;
};

exports.isRestartingFrequently = async () => {
  let answer = null;
  await inquirer
    .prompt({
      type: 'confirm',
      name: 'isRestartingFrequently',
      message: `Is the Pod restarting frequently? Cycling between ${colors.red(
        'Running'
      )} and ${colors.red('CrashLoopBackoff')}?`,
      default: 'Y',
    })
    .then((ans) => (answer = ans.isRestartingFrequently));
  return answer;
};

exports.serviceName = async () => {
  let answer = null;
  await inquirer
    .prompt({
      type: 'input',
    ...
    .then((ans) => (answer = ans.serviceName));
  return answer;
};

exports.podPort = async () => {
  let answer = null;
  await inquirer
    .prompt({
      type: 'input',
      name: 'podPort',
      message: 'What is the pod port number?',
    })
    .then((ans) => (answer = ans.podPort));
  return answer;
};
